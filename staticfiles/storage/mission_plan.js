    ...
    missionPlanContainer.innerHTML = data.map(plan => `
        <div x-data="{ 
            ...
            <div x-show="openPlanModal"
                ...
                <div @click.away="openPlanModal = false" class="bg-white rounded-lg max-w-6xl w-full p-6 shadow-lg space-y-2">
                    <!-- Modal Header -->
                    <div class="modal-header p-4 bg-rifleBlue text-rifleBlue-100 rounded-md">
                        <span class="flex flex-row">
                            <h4 class="mr-3 font-bold">${plan.satellite || "Unknown Satellite"}</h4>
                            <p>(${new Date(plan.rise_time).toLocaleDateString('en-US', {
                            month: 'short',
                    ...
                    <div id="map-${plan.id}" class="flex flex-col h-[55vh] rounded-md">
